...
//  Tlačidlá (pull-down na GND cez 10kΩ,
alebo INPUT_PULLUP):
...
const int PIN_BTN3 = 6;   // Boost /
špeciálna akcia

const int PIN_BTN4 = 7;   // Reset /
Brzda



// --- Kalibrácia stredov (zmeraj pri štarte) ---
...
  // Priemerujeme 20 odčítaní pri štarte
(joysticky musia byť v strede!)
...
  // Tlačidlá: INPUT_PULLUP = stlačené =
LOW, uvoľnené = HIGH
...
  // Kalibruj stredy - drž joysticky v
neutrálnej polohe pri zapnutí!
...
  if (now - lastSendTime <
SEND_INTERVAL_MS) return;

  lastSendTime = now;



  // --- Čítanie analógových hodnôt
---
...
  // --- Čítanie tlačidiel (LOW =
stlačené → invertujeme na 1) ---

  int lsw = !digitalRead(PIN_LEFT_SW);

  int rsw =
!digitalRead(PIN_RIGHT_SW);

  int b1 
= !digitalRead(PIN_BTN1);

  int b2 
= !digitalRead(PIN_BTN2);

  int b3 
= !digitalRead(PIN_BTN3);

  int b4 
= !digitalRead(PIN_BTN4);
...
//  Ľavý joystick X (LX) → Yaw  (otáčanie dronu vľavo/vpravo)

//  Ľavý joystick Y (LY) → Throttle
(výška - tlač hore = lieta hore)

//  Pravý joystick X (RX) → Roll / Strafe
(pohyb do strany)

//  Pravý joystick Y (RY) → Pitch
(dopredu / dozadu)

//  BTN1 → Zvýšiť rýchlosť

//  BTN2 → Znížiť rýchlosť

//  BTN3 → Speed Boost (turbo)

//  BTN4 → Brzda / Reset pohybu

//  LSW 
→ Ľavý joystick stlačený (voliteľné)

//  RSW 
→ Pravý joystick stlačený (voliteľné)
